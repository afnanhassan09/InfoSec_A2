...
def issue_certificate(
    ca_name: str, cert_name: str, cert_type: str, output_dir: str = "certs"
):
    ...
    if not os.path.exists(ca_key_path) or not os.path.exists(ca_cert_path):
        raise FileNotFoundError(
            "CA key or certificate file not found. Please generate the CA first."
        )
    ...
    cert_builder = (
        ...
        .add_extension(
            x509.SubjectAlternativeName([x509.DNSName(cert_name)]), critical=False
        )
        .add_extension(
            x509.KeyUsage(
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "ca_name", type=str, help="Name of the Root CA (used in file names)."
    )
    parser.add_argument(
        "cert_name", type=str, help="Common Name (CN) for the new certificate."
    )
    parser.add_argument(
        "cert_type",
        type=str,
        choices=["server", "client"],
        help="Type of certificate to issue (server/client).",
    )
    parser.add_argument(
        "--output_dir",
        type=str,
        default="certs",
        help="Directory to save the issued certificate and key.",
    )
